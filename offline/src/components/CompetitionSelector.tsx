...
export default function CompetitionSelector({ isFloating = false }: CompetitionSelectorProps) {
  return (
    <motion.div
      ...
      className="bg-[rgba(12,12,12,0.16)] backdrop-blur-md border border-[rgba(255,255,255,0.06)] rounded-[20px] shadow-lg overflow-hidden transition-all duration-500"
    >
        ...
        {competitions.map((comp, index) => (
          ...
          >
            <img 
              src={comp.logo} 
              alt={comp.alt} 
              className="object-contain group-hover:scale-[1.08] transition-transform duration-250 w-10 h-10"
              loading="lazy"
              onError={(e) => console.error(`Failed to load logo: ${comp.logo}`, e)}
            />
            <span className="font-medium text-white uppercase tracking-wide text-[11px]">
              {comp.name}
            </span>
